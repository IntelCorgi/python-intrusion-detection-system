    ...
    if dstIPandPort in non_handshake_syn_flood:
        ...
        while len(packets) > 0:
            ...
            if timeStamp - current_packet['timeStamp'] > 1:
                ...
                break
        packets.append({'source': ethData.src, 'destination': socket.inet_ntoa(ethData.dst), 'timeStamp': timeStamp, 'packetNum': pktCount})
        if len(packets) > 100:
...
def testPS(IP, port, pktCount):
    ...
    if destination in portscan_attempts:
        ...
        portscan_attempts[destination] = [{'source': IP.src, 'destination': destination, 'destinationPort':port, 'packetNum': pktCount}]
